# 🚀🚀🚀auto-youth-study
- 👍青年大学习自动完成观看记录，秒完成脚本
- [Go语言版本见这里v1.0](https://github.com/JimouChen/auto-youth-study-go)
...
```
里面保存观看记录
> 拿到token(X-Litemall-Token)和章节id(ChapterId)就可以去发送post请求触发刷新观看记录
- 章节id可以直接get获取   get
- token获取方法：
    - 需要获取mid  (登录时候必要的)
    - 通过mid获取sign   get
...
- 所以原理就是只要模拟手动保存观看记录即可，也就是不需要全部看完整个视频才有记录，点进去看了一两秒即可退出就是已学的状态了，代码体现在发生post请求到服务端，服务端内部修改是否有观看记录/历史的字段为已观看状态
